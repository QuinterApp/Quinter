...
class general(wx.Panel, wx.Dialog):
	def __init__(self, parent):
		...
		self.earcon_audio=wx.CheckBox(self, -1, "Play a sound when a tweet contains audio that is playable by either the client or an external media player")
...
class advanced(wx.Panel, wx.Dialog):
	def __init__(self, parent):
		...
		self.media_player_box=wx.StaticBox(self, -1,"External Media Player")
