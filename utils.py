...
def lookup_user_name(account,name,use_api=True):
	...
	if not use_api:
		return -1
	try:
		user=account.api.lookup_users(screen_name=[name])[0]
...
def get_user_objects_in_tweet(account,status,exclude_self=False,exclude_orig=False):
	...
	if hasattr(status,"message_create"):
		users.append(lookup_user(user_id=status.message_create['sender_id']))
		users.append(lookup_user(user_id=status.message_create['target']['recipient_id']))
		return users
